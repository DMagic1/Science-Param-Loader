...
namespace ScienceParamModifier.Framework
{
    ...
    public abstract class SM_MBW : SM_MBE
    {
		...
		internal void DrawGUI()
		{
			...
			GUI.skin = SM_SkinsLibrary.CurrentSkin;
        ...
        private void DrawToolTip()
        {
            ...
            if (TooltipsEnabled && Visible && (strToolTipText != "") && ((TooltipDisplayForSecs == 0) || (fltTooltipTime < (Single)TooltipDisplayForSecs)))
            {
                ...
                GUIStyle styleTooltip = SM_SkinsLibrary.CurrentTooltip;
                ...
                if (TooltipMaxWidth > 0)
                {
                    ...
                    SM_SkinsLibrary.CurrentTooltip.CalcMinMaxWidth(contTooltip, out minwidth, out maxwidth); // figure out how wide one line would be
                    _TooltipPosition.width = Math.Min(TooltipMaxWidth - SM_SkinsLibrary.CurrentTooltip.padding.horizontal, maxwidth); //then work out the height with a max width
                    _TooltipPosition.height = SM_SkinsLibrary.CurrentTooltip.CalcHeight(contTooltip, TooltipPosition.width); // heres the result
                }
                else
                {
                    //calc the new width and height
                    Vector2 Size = SM_SkinsLibrary.CurrentTooltip.CalcSize(contTooltip);
